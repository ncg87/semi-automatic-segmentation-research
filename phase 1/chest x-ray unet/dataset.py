...
class ChestXRaysSegmentationDataset(Dataset):
    ...
    def __init__(self, image_path, mask_path, image_size = 256, augumented = False):
        ...
        self.images = sorted(tqdm(glob.glob(os.path.join(image_path,"*.png"))))
    ...
    def __getitem__(self, idx):
        ...
        #print(self.images[idx],self.masks[idx])
        #print(img_rescaled.shape, mask_rescaled.shape)
        
        # Add 3rd dim for channels if not already there
        if(img.ndim != 3):
            img_rescaled = np.stack([img_rescaled,img_rescaled,img_rescaled],2)
